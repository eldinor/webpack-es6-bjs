...
export class Clean3 implements CreateSceneClass {
...
export class Drone extends YUKA.Vehicle {
    ...
    constructor(
        ...
        this.stateMachine.add(CONSTRUCT, new ConstructState());
...
class ConstructState extends YUKA.State<YUKA.Vehicle> {
    enter(drone: any) {
        console.log("ENTER ConstructState");
        droneUI!.innerHTML = "ConstructState";
        // drone.velocity = new YUKA.Vector3(0, 0, 0);
        //  console.log(drone);
        drone.steering.behaviors[0].active = false;
        drone.steering.behaviors[0].target = drone.client.position;
    ...
    execute(drone: any) {
        ...
        if (drone.currentTime > 3) {
            drone.stateMachine.changeTo(RETURN);
            drone.currentTime = 0;
            drone.steering.behaviors[0].active = true;
            drone.steering.behaviors[0].target = drone.warehouse.position;
            drone.client = undefined;
            console.log("RETURN!");
        }
    }
